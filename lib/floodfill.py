...
def new_full(value, dimensions=(N, N), value_type='uint16'):
    tile = np.empty(dimensions, value_type)
    tile.fill(value)
    return tile
...
def morph(offset, tiles, full_opaque):
    ...
    if num_workers > 1:
        ...
        for _ in range(num_workers):
            ...
            for tile_coord, tile in result.iteritems():
...
def flood_fill(
    ...
    tiles_to_composite = filled.iteritems()
...
def scanline_fill(
    ...
    def uniform_tile(alpha):
        ...
        if alpha not in uniform_tiles:
            uniform_tiles[alpha] = new_full(alpha)
